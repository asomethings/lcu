import { LcuOptions } from './interface/lcu-options.interface'

export const DEFAULT_LCU_PROTOCOL = 'https'
export const DEFAULT_LCU_WEBSOCKET_URL = '127.0.0.1'
export const DEFAULT_LCU_URL = '127.0.0.1'
export const DEFAULT_LCU_TIMEOUT = 1000
export const DEFAULT_LCU_CONFIG: LcuOptions = {
  protocol: DEFAULT_LCU_PROTOCOL,
  websocketUrl: DEFAULT_LCU_WEBSOCKET_URL,
  url: DEFAULT_LCU_URL,
  timeout: DEFAULT_LCU_TIMEOUT,
}

export const RIOT_GAMES_CERTIFICATE = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
